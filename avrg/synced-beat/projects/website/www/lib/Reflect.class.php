...
class Reflect {
	...
	static function callMethod($o, $func, $args) {
		...
		if(is_string($o)) {
			if($args->length === 0) {
				...
				if($args->length === 1) {
					$»tmp2 = call_user_func(Reflect::field($o, $func), $args[0]);
					$GLOBALS['%s']->pop();
					return $»tmp2;
				}
				else {
					$»tmp3 = call_user_func(Reflect::field($o, $func), $args[0], $args[1]);
					$GLOBALS['%s']->pop();
					return $»tmp3;
				}
			}
		...
		{
			$»tmp4 = call_user_func_array(is_callable($func) ? $func : array($o, $func) , $args == null ? array() : $args->»a);
			$GLOBALS['%s']->pop();
			return $»tmp4;
		}
		$GLOBALS['%s']->pop();
	...
	static function fields($o) {
		...
		{
			$»tmp2 = ($o instanceof _hx_array ? new _hx_array(array('concat','copy','insert','iterator','length','join','pop','push','remove','reverse','shift','slice','sort','splice','toString','unshift')) : ((is_string($o) ? new _hx_array(array('charAt','charCodeAt','indexOf','lastIndexOf','length','split','substr','toLowerCase','toString','toUpperCase')) : new _hx_array(array_keys(get_object_vars($o))))));
			$GLOBALS['%s']->pop();
			return $»tmp2;
		}
		$GLOBALS['%s']->pop();
