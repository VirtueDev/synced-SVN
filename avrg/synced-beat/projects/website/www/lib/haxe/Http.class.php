...
class haxe_Http {
	...
	public function request($post) {
		...
		$err = false;
		$this->onError = array(new _hx_lambda(array("err" => &$err, "me" => &$me, "me1" => &$me1, "old" => &$old, "output" => &$output, "post" => &$post, "»spos" => &$»spos), null, array('e'), "{
			\$GLOBALS['%s']->push(\"haxe.Http::request@246\");
			\$»spos2 = \$GLOBALS['%s']->length;
			\$err = true;
	...
	public function readChunk($chunk_re, $api, $buf, $len) {
		...
		if($len >= $end) {
			...
			{
				$»tmp2 = $this->readChunk($chunk_re, $api, $buf->sub($end, $len), $len);
				$GLOBALS['%s']->pop();
				return $»tmp2;
			}
		}
	...
	static function requestUrl($url) {
		...
		$h = new haxe_Http($url);
		$r = null;
		$h->onData = array(new _hx_lambda(array("h" => &$h, "r" => &$r, "url" => &$url, "»spos" => &$»spos), null, array('d'), "{
			\$GLOBALS['%s']->push(\"haxe.Http::requestUrl@619\");
			\$»spos2 = \$GLOBALS['%s']->length;
			\$r = \$d;
			\$GLOBALS['%s']->pop();
		}"), 'execute1');
		$h->onError = array(new _hx_lambda(array("h" => &$h, "r" => &$r, "url" => &$url, "»spos" => &$»spos), null, array('e'), "{
			\$GLOBALS['%s']->push(\"haxe.Http::requestUrl@622\");
			\$»spos2 = \$GLOBALS['%s']->length;
