...
class php_Web {
	...
	static function getParamValues($param) {
		...
		$explore = array(new _hx_lambda(array("explore" => &$explore, "param" => &$param, "reg" => &$reg, "res" => &$res, "製pos" => &$製pos), null, array('data'), "{
			\$GLOBALS['%s']->push(\"php.Web::getParamValues@43\");
	...
	static function getMultipart($maxSize) {
		...
		$buf = null;
		$curname = null;
		php_Web::parseMultipart(array(new _hx_lambda(array("buf" => &$buf, "curname" => &$curname, "h" => &$h, "maxSize" => &$maxSize, "製pos" => &$製pos), null, array('p','_'), "{
			\$GLOBALS['%s']->push(\"php.Web::getMultipart@274\");
			\$製pos2 = \$GLOBALS['%s']->length;
			if(\$curname !== null) {
				\$h->set(\$curname, \$buf->b);
			}
			...
			\$GLOBALS['%s']->pop();
		}"), 'execute2'), array(new _hx_lambda(array("buf" => &$buf, "curname" => &$curname, "h" => &$h, "maxSize" => &$maxSize, "製pos" => &$製pos), null, array('str','pos','len'), "{
			\$GLOBALS['%s']->push(\"php.Web::getMultipart@282\");
			\$製pos2 = \$GLOBALS['%s']->length;
			\$maxSize -= \$len;
			if(\$maxSize < 0) {
				throw new HException(\"Maximum size reached\");
